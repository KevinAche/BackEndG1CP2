...
CREATE VIEW alumnos_solicitudes_aceptadas AS select a.id_alumno, al.cedula, al.primer_nombre||' '||al.segundo_nombre as "a_nombres", al.primer_apellido||' '||al.segundo_apellido as "a_apellidos", se.id_solicitud_empresa, se.id_responsableppp, rp.cedula as "cedula_rp",d.abrev_titulo as "tit_rp" , rp.primer_nombre||' '||rp.segundo_nombre as "rp_nombres", rp.primer_apellido||' '||rp.segundo_apellido as "rp_apellidos", car.nombre as "carrera_rp", sa.estado as "estado_solicitud", e.id_empresa, e.nombre_empresa  from solicitud_alumno sa JOIN alumno a on a.id_alumno = sa.id_alumno and sa.id_alumno not in (select id_alumno from tutor_empresarial) JOIN carreras car on a.id_carrera = car.id_carrera JOIN convocatoria c on c.id_convocatoria =sa.id_convocatoria  JOIN solicitudes_empresa se ON c.id_solicitud_empresa = se.id_solicitud_empresa JOIN responsable_ppp resp on resp.id_responsableppp= se.id_responsableppp JOIN docente d on d.id_docente = resp.id_docente JOIN persona rp on rp.id_persona= d.id_persona JOIN personal_empresa pe on pe.id_personal = se.id_empleado JOIN empresa e on e.id_empresa = pe.id_empresa  JOIN persona al on al.id_persona = a.id_alumno where upper(sa.estado) like '%ACEPTADO%' OR upper(sa.estado) like '%ACEPTADA%';
CREATE VIEW informes_finales_estudiantes AS SELECT if.id_informe_final,al.id_alumno, if.fecha_emision, if.estado, p.primer_nombre||' '||p.segundo_nombre as "a_nombres", p.primer_apellido||' '||p.segundo_apellido as "a_apellidos", p.cedula FROM informe_final if JOIN alumno al ON if.id_alumno = al.id_alumno JOIN persona p ON p.id_persona = al.id_persona;
