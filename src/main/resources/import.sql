...
CREATE VIEW lista_actividades_empresa AS SELECT ac.id_actividad, ac.descripcion, ac.area, e.id_empresa, e.nombre_empresa, p.primer_nombre||' '||p.segundo_nombre as "nombresrp", p.primer_apellido||' '||p.segundo_apellido as "apellidosrp", d.titulo, d.abrev_titulo FROM actividades ac JOIN convenios c ON ac.id_convenio = c.id_convenio JOIN personal_empresa pe ON pe.id_personal = c.id_gerente JOIN empresa e ON e.id_empresa = pe.id_empresa JOIN responsable_ppp rp ON rp.id_responsableppp = c.id_responsableppp JOIN docente d ON d.id_docente = rp.id_docente JOIN persona p ON p.id_persona = d.id_persona;
